...
const ManagerDashboard = ({ bookings, onUpdateStatus }: ManagerDashboardProps) => {
  ...
  const weekDays = Array.from({ length: 7 }, (_, i) => {
    const d = addDays(
      subDays(selectedDate, selectedDate.getDay() === 0 ? 6 : selectedDate.getDay() - 1),
      i
    );
    return d;
  });
    ...
    <div className="min-h-screen bg-background safe-top safe-bottom">
      ...
      <div className="px-6 pt-6 pb-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-display font-semibold text-foreground">{user?.name}</h1>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[
              ...
              { icon: Clock, label: "In attesa", value: pendingCount, color: "text-warning" },
                ...
                <div key={booking.id} className="p-4 rounded-2xl bg-card border border-border shadow-card">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {format(new Date(booking.date), "d MMM", { locale: it })} • {booking.time}
                    </span>
                  </div>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{booking.userName}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium text-foreground">{booking.serviceName}</p>
                      <p className="text-xs text-muted-foreground">{booking.duration} min • €{booking.price}</p>
                    </div>
                  </div>
                </div>
              ))}
